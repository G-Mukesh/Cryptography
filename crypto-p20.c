...
int main() {
    ...
    for ( i = 0; i < BLOCK_SIZE; i++) printf("%02X ", (unsigned char)ciphertext_ecb1[i]);
    printf("\nCiphertext Block 2 (ECB): ");
    for ( i = 0; i < BLOCK_SIZE; i++) printf("%02X ", (unsigned char)ciphertext_ecb2[i]);
    printf("\n");
     printf("\nCBC Mode Encryption (with error in block 1):\n");
    printf("Ciphertext Block 1 (CBC): ");
    for ( i = 0; i < BLOCK_SIZE; i++) printf("%02X ", (unsigned char)ciphertext_cbc1[i]);
    printf("\nCiphertext Block 2 (CBC): ");
    for ( i = 0; i < BLOCK_SIZE; i++) printf("%02X ", (unsigned char)ciphertext_cbc2[i]);
    printf("\n");
